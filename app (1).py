...
# ══════════════════════════════════════════════════════════════════════════════
DEFAULT_EAST = [
    # (name, seed, net_rating)
    ("Detroit Pistons",        1,  0.0),   # ← syötä NR
...
with tab2:
    ...
    with c1:
        ...
        sa_name = st.text_input("Joukkue A", value="Cleveland Cavaliers", key="sa_n")
        sa_nr   = st.number_input("Net Rating", value=8.5, step=0.1, format="%.1f", key="sa_nr")
    with c2:
        ...
        sb_nr   = st.number_input("Net Rating", value=6.8, step=0.1, format="%.1f", key="sb_nr")
...
""",unsafe_allow_html=True)
    ...
    for i,(col,(dname,dnr)) in enumerate(zip(pi_cols,defs)):
        with col:
            ...
            n  = st.text_input("",value=dname,key=f"pi_n{i}_{conf_pi}",label_visibility="collapsed")
            nr = st.number_input("NR",value=dnr,step=0.1,format="%.1f",key=f"pi_nr{i}_{conf_pi}",label_visibility="collapsed")
